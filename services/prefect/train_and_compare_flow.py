# services/prefect/train_and_compare_flow.py
...
def make_pipeline(df: pd.DataFrame, seed: int):
    cat_cols = [
        ...
        if df[c].dtype == "object" and c not in ["user_id", "event_timestamp", "churn_label"]
    ]
    num_cols = [
        c for c in df.columns
...
def train_candidate(as_of: str, seed: int) -> dict:
    ...
    # robust: prendre la plus grande version
    candidate_version = max(latest_none, key=lambda mv: int(mv.version)).version
...
def train_and_compare_flow(as_of: str, seed: int = 42, delta: float = 0.01):
    """
    Entraîne un modèle candidat sur as_of, évalue Production sur as_of,
    compare val_auc et promeut si nécessaire.
    """
