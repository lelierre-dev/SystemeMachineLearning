...
def main():
    ...
    # TODO 2: définir la liste des features Feast à récupérer (liste de strings).
    features = [
    ...
    print("N rows after merge:", len(df))
    ...
    with mlflow.start_run(run_name=f"rf_baseline_{AS_OF}") as run:
        mlflow.log_param("n_rows_after_merge", int(len(df)))
        start = time.time()
        ...
        mlflow.sklearn.log_model(
            sk_model=pipe,  # on log la pipeline complète (prep + modèle)
            artifact_path="model",
